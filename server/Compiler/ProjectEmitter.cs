...
public static class ProjectEmitter
{
    ...
    public static Dictionary<string, string> EmitProject(Blueprint bp, string? gamePath = null)
    {
        ...
        var (eventCode, registerItems, registerRecipes, registerBuildings, registerTiles, registerLocales, registerLiquids, statuses, registerCreatures, functions) = ExtractRegisterMarkers(bp.EventHandlers!);
        ...
        if (registerCreatures != null && registerCreatures.Count > 0)
            files["CuCreatureManager.cs"] = EmitCuCreatureManagerFile(ns);

        return files;
    }

    private static (string eventCode, List<ItemEntry> items, List<RecipeEntry> recipes, List<BuildingEntry> buildings, List<TileEntry> tiles, List<LocaleEntry> locales, List<LiquidEntry> liquids, List<StatusEntry> statuses, List<CreatureEntry> creatures, List<FunctionDef> functions) ExtractRegisterMarkers(string eventCode)
    {
        ...
        foreach (var raw in lines)
        {
            ...
            {
                var json = line.Substring(20).Trim();
                try { var cr = JsonSerializer.Deserialize<CreatureEntry>(json); if (cr != null) creatures.Add(cr); } catch { }
            }
        ...
        return (string.Join('\n', remainingLines), items, recipes, buildings, tiles, locales, liquids, statuses, creatures, functions);
    }
...
";
    ...
    private static string EmitCuCreatureManagerFile(string ns)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// AUTO-GENERATED by CuBlocky. Creature runtime manager.");
        sb.AppendLine("using UnityEngine;");
        sb.AppendLine("using System.Collections.Generic;");
        sb.AppendLine();
        sb.AppendLine($"namespace {ns}");
        sb.AppendLine("{");
        sb.AppendLine(CodeEmitter.EmitCuCreatureManager());
        sb.AppendLine("}");
        return sb.ToString();
    }
}
