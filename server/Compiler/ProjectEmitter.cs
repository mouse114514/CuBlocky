...
";
    ...
    private static string EmitCuUI(string ns, List<UIControlEntry> controls)
    {
        ...
        sb.AppendLine("            public string Sprite = \"\";");
        sb.AppendLine("            public float Value = 0f;");
        sb.AppendLine("            public float Min = 0f;");
        sb.AppendLine("            public float Max = 1f;");
        sb.AppendLine("            public Color FillColor = new Color(0.298f, 0.686f, 0.314f);");
        sb.AppendLine("            public string[] Options = new string[0];");
        sb.AppendLine("            public int SelectedIndex = 0;");
        sb.AppendLine("            public bool DropdownOpen = false;");
        sb.AppendLine("            public string AlignH = \"center\";");
        sb.AppendLine("            public string AlignV = \"center\";");
        sb.AppendLine("        }");
        sb.AppendLine();
        ...
        sb.AppendLine("        private static string _activeSlider = null;");
        ...
        sb.AppendLine("        private static Dictionary<string, Sprite> _spriteCache = new();");
        ...
        foreach (var c in controls)
        {
            ...
            var sprite = (c.Sprite ?? "").Replace("\"", "\\\"");
            var fill = HexToColor(c.FillColor);
            var options = string.Join("\\n", (c.Options ?? "").Split('|').Select(o => o.Replace("\"", "\\\"")));
            var tc = HexToColor(c.TextColor);
            var bg = HexToColor(c.BackgroundColor);
            var sc = HexToColor(c.StrokeColor);
            sb.AppendLine($"            _controls[\"{id}\"] = new Ctrl {{ Id = \"{id}\", Type = \"{c.Type}\", X = {c.X}f, Y = {c.Y}f, Width = {c.Width}f, Height = {c.Height}f, Text = \"{text}\", FontSize = {c.FontSize}, TextColor = {tc}, BgColor = {bg}, StrokeColor = {sc}, StrokeWidth = {c.StrokeWidth}f, CornerRadius = {c.CornerRadius}f, Opacity = {c.Opacity}f, Visible = {c.Visible.ToString().ToLower()}, TextFieldText = \"{text}\", Sprite = \"{sprite}\", Value = {c.Value}f, Min = {c.Min}f, Max = {c.Max}f, FillColor = {fill}, Options = \"{options}\".Split('\\n'), AlignH = \"{c.AlignH}\", AlignV = \"{c.AlignV}\" }};");
        }
        ...
        sb.AppendLine("                case \"value\": c.Value = ParseFloat(val); break;");
        sb.AppendLine("                case \"min\": c.Min = ParseFloat(val); break;");
        sb.AppendLine("                case \"max\": c.Max = ParseFloat(val); break;");
        sb.AppendLine("                case \"fillColor\": c.FillColor = ParseColor(s); break;");
        sb.AppendLine("                case \"sprite\": c.Sprite = s; break;");
        sb.AppendLine("                case \"options\": c.Options = s.Split('|'); c.SelectedIndex = Mathf.Clamp(c.SelectedIndex, 0, Mathf.Max(0, c.Options.Length - 1)); break;");
        sb.AppendLine("                case \"selected\": c.SelectedIndex = (int)ParseFloat(val); if (c.Options.Length > 0) c.SelectedIndex = Mathf.Clamp(c.SelectedIndex, 0, c.Options.Length - 1); break;");
        sb.AppendLine("                case \"alignH\": c.AlignH = s; break;");
        sb.AppendLine("                case \"alignV\": c.AlignV = s; break;");
        sb.AppendLine("            }");
        sb.AppendLine("        }");
        sb.AppendLine();
        ...
        sb.AppendLine("        {");
        sb.AppendLine("            InitData();");
        sb.AppendLine("            if (_controls.TryGetValue(id, out var c) && c.Type == \"dropdown\" && c.Options.Length > 0)");
        sb.AppendLine("                return c.SelectedIndex >= 0 && c.SelectedIndex < c.Options.Length ? c.Options[c.SelectedIndex] : \"\";");
        sb.AppendLine("            return _controls.TryGetValue(id, out var c2) ? c2.TextFieldText : \"\";");
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        public static float GetValue(string id)");
        sb.AppendLine("        {");
        sb.AppendLine("            InitData();");
        sb.AppendLine("            return _controls.TryGetValue(id, out var c) ? c.Value : 0f;");
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        public static bool GetToggle(string id)");
        sb.AppendLine("        {");
        sb.AppendLine("            InitData();");
        sb.AppendLine("            return _controls.TryGetValue(id, out var c) && c.ToggleState;");
        sb.AppendLine("        }");
        sb.AppendLine();
        ...
        sb.AppendLine("                int ah = c.AlignH == \"left\" ? 0 : c.AlignH == \"right\" ? 2 : 1;");
        sb.AppendLine("                int av = c.AlignV == \"top\" ? 0 : c.AlignV == \"bottom\" ? 2 : 1;");
        sb.AppendLine("                _labelStyle.alignment = (TextAnchor)(av * 3 + ah);");
        sb.AppendLine();
        ...
        sb.AppendLine("                }");
        sb.AppendLine("                else if (c.Type == \"label\")");
        sb.AppendLine("                    GUI.Label(rect, c.Text, _labelStyle);");
        sb.AppendLine("                else if (c.Type == \"box\")");
        sb.AppendLine("                {");
        sb.AppendLine("                    if (c.Text.Length > 0) GUI.Label(rect, c.Text, _labelStyle);");
        sb.AppendLine("                }");
        ...
        sb.AppendLine("                else if (c.Type == \"image\")");
        sb.AppendLine("                {");
        sb.AppendLine("                    var spr = GetSprite(c.Sprite);");
        sb.AppendLine("                    if (spr != null) GUI.DrawTexture(rect, spr.texture, ScaleMode.ScaleToFit);");
        sb.AppendLine("                }");
                sb.AppendLine("                else if (c.Type == \"slider\")");
        sb.AppendLine("                {");
        sb.AppendLine("                    float span = c.Max - c.Min;");
        sb.AppendLine("                    float frac = span > 0f ? Mathf.Clamp01((c.Value - c.Min) / span) : 0f;");
        sb.AppendLine("                    var ev = Event.current;");
        sb.AppendLine("                    if (ev != null)");
        sb.AppendLine("                    {");
        sb.AppendLine("                        if (ev.type == EventType.MouseDown && rect.Contains(ev.mousePosition)) { _activeSlider = c.Id; ev.Use(); }");
        sb.AppendLine("                        if (_activeSlider == c.Id && ev.type == EventType.MouseDrag)");
        sb.AppendLine("                        {");
        sb.AppendLine("                            frac = Mathf.Clamp01((ev.mousePosition.x - rect.x) / Mathf.Max(1f, rect.width));");
        sb.AppendLine("                            c.Value = c.Min + frac * span;");
        sb.AppendLine("                            ev.Use();");
        sb.AppendLine("                        }");
        sb.AppendLine("                        if (_activeSlider == c.Id && ev.type == EventType.MouseUp) { _activeSlider = null; ev.Use(); }");
        sb.AppendLine("                    }");
        sb.AppendLine("                    frac = span > 0f ? Mathf.Clamp01((c.Value - c.Min) / span) : 0f;");
        sb.AppendLine("                    var fillRect = new Rect(rect.x, rect.y, rect.width * frac, rect.height);");
        sb.AppendLine("                    if (fillRect.width > 0.5f) GUI.DrawTexture(fillRect, GetTex(c.FillColor));");
        sb.AppendLine("                    float tw = Mathf.Max(6f, rect.height);");
        sb.AppendLine("                    GUI.DrawTexture(new Rect(rect.x + (rect.width - tw) * frac, rect.y, tw, rect.height), GetTex(c.TextColor));");
        sb.AppendLine("                }");
        sb.AppendLine("                else if (c.Type == \"progressbar\")");
        sb.AppendLine("                {");
        sb.AppendLine("                    float span = c.Max - c.Min;");
        sb.AppendLine("                    float frac = span > 0f ? Mathf.Clamp01((c.Value - c.Min) / span) : 0f;");
        sb.AppendLine("                    var fillRect = new Rect(rect.x, rect.y, rect.width * frac, rect.height);");
        ...
        sb.AppendLine("                    if (c.Text.Length > 0) GUI.Label(rect, c.Text, _labelStyle);");
        sb.AppendLine("                }");
        sb.AppendLine("                else if (c.Type == \"dropdown\")");
        sb.AppendLine("                {");
        sb.AppendLine("                    string cur = c.Options.Length > 0 && c.SelectedIndex >= 0 && c.SelectedIndex < c.Options.Length ? c.Options[c.SelectedIndex] : \"\";");
        sb.AppendLine("                    if (GUI.Button(rect, cur, _labelStyle)) c.DropdownOpen = !c.DropdownOpen;");
        sb.AppendLine("                    if (c.DropdownOpen && c.Options.Length > 0)");
        sb.AppendLine("                    {");
        sb.AppendLine("                        float oh = rect.height;");
        sb.AppendLine("                        var listRect = new Rect(rect.x, rect.yMax, rect.width, oh * c.Options.Length);");
        sb.AppendLine("                        GUI.Box(new Rect(listRect.x, listRect.y, listRect.width, listRect.height + 2f), \"\", _bgStyle);");
        sb.AppendLine("                        for (int i = 0; i < c.Options.Length; i++)");
        sb.AppendLine("                        {");
        sb.AppendLine("                            var or = new Rect(rect.x, rect.yMax + i * oh, rect.width, oh);");
        sb.AppendLine("                            if (GUI.Button(or, c.Options[i], _labelStyle))");
        sb.AppendLine("                            {");
        sb.AppendLine("                                c.SelectedIndex = i;");
        sb.AppendLine("                                c.DropdownOpen = false;");
        sb.AppendLine("                            }");
        sb.AppendLine("                        }");
        sb.AppendLine("                    }");
        sb.AppendLine("                }");
        ...
        sb.AppendLine("            if (r <= 0) return GetTex(Color.white);");
        sb.AppendLine("            if (_nineSliceCache.TryGetValue(r, out var cached)) return cached;");
        sb.AppendLine("            int size = r * 2 + 4;");
        ...
        sb.AppendLine("                {");
        sb.AppendLine("                    bool inCornerZone = (x < r && y < r) || (x >= size - r && y < r) || (x < r && y >= size - r) || (x >= size - r && y >= size - r);");
        sb.AppendLine("                    if (!inCornerZone) { px[y * size + x] = white; continue; }");
        sb.AppendLine("                    float cx, cy;");
        sb.AppendLine("                    if (x < r && y < r) { cx = r - 1 - x; cy = r - 1 - y; }");
        sb.AppendLine("                    else if (x >= size - r && y < r) { cx = x - (size - r); cy = r - 1 - y; }");
        sb.AppendLine("                    else if (x < r && y >= size - r) { cx = r - 1 - x; cy = y - (size - r); }");
        sb.AppendLine("                    else { cx = x - (size - r); cy = y - (size - r); }");
        sb.AppendLine("                    px[y * size + x] = (cx * cx + cy * cy <= r * r) ? white : transparent;");
        sb.AppendLine("                }");
        sb.AppendLine("            }");
        sb.AppendLine("            tex.SetPixels32(px);");
        sb.AppendLine("            tex.Apply();");
        sb.AppendLine("            _nineSliceCache[r] = tex;");
        ...
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        static Sprite GetSprite(string name)");
        sb.AppendLine("        {");
        sb.AppendLine("            if (string.IsNullOrEmpty(name)) return null;");
        sb.AppendLine("            if (_spriteCache.TryGetValue(name, out var cached)) return cached;");
        sb.AppendLine("            Sprite s = null;");
        sb.AppendLine("            try { s = AssetLoader.LoadEmbeddedSprite(name); } catch { }");
        sb.AppendLine("            _spriteCache[name] = s;");
