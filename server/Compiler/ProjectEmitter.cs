...
";
    ...
    private static string EmitCuUI(string ns, List<UIControlEntry> controls)
    {
        ...
        sb.AppendLine("            public Rect Rect;");
        ...
        sb.AppendLine("        private static bool _init = false;");
        sb.AppendLine("        private static GUIStyle _style;");
        sb.AppendLine();
        sb.AppendLine("        public static void Init()");
        sb.AppendLine("        {");
        sb.AppendLine("            if (_init) return;");
        sb.AppendLine("            _init = true;");
        sb.AppendLine("            _style = new GUIStyle(GUI.skin.button);");
        sb.AppendLine();
        ...
        foreach (var c in controls)
        {
            ...
            sb.AppendLine($"            _controls[\"{id}\"] = new Ctrl {{ Id = \"{id}\", Type = \"{c.Type}\", Rect = new Rect({c.X}f, {c.Y}f, {c.Width}f, {c.Height}f), Text = \"{text}\", FontSize = {c.FontSize}, TextColor = {tc}, BgColor = {bg}, Opacity = {c.Opacity}f, Visible = {c.Visible.ToString().ToLower()}, TextFieldText = \"{text}\" }};");
        }
        ...
        sb.AppendLine("                case \"x\": c.Rect = new Rect(ParseFloat(val), c.Rect.y, c.Rect.width, c.Rect.height); break;");
        sb.AppendLine("                case \"y\": c.Rect = new Rect(c.Rect.x, ParseFloat(val), c.Rect.width, c.Rect.height); break;");
        sb.AppendLine("                case \"width\": c.Rect = new Rect(c.Rect.x, c.Rect.y, ParseFloat(val), c.Rect.height); break;");
        sb.AppendLine("                case \"height\": c.Rect = new Rect(c.Rect.x, c.Rect.y, c.Rect.width, ParseFloat(val)); break;");
        ...
        sb.AppendLine("                GUI.color = new Color(c.TextColor.r, c.TextColor.g, c.TextColor.b, c.Opacity);");
        sb.AppendLine("                GUI.backgroundColor = c.BgColor;");
        sb.AppendLine("                _style.fontSize = c.FontSize;");
        sb.AppendLine("                _style.normal.textColor = c.TextColor;");
        sb.AppendLine("                if (c.Type == \"button\")");
        sb.AppendLine("                {");
        sb.AppendLine("                    _pressed[c.Id] = GUI.Button(c.Rect, c.Text, _style);");
        sb.AppendLine("                }");
        sb.AppendLine("                else if (c.Type == \"textfield\")");
        sb.AppendLine("                {");
        sb.AppendLine("                    c.TextFieldText = GUI.TextField(c.Rect, c.TextFieldText, _style);");
        sb.AppendLine("                }");
        sb.AppendLine("                else if (c.Type == \"toggle\")");
        sb.AppendLine("                {");
        sb.AppendLine("                    c.ToggleState = GUI.Toggle(c.Rect, c.ToggleState, c.Text, _style);");
        sb.AppendLine("                }");
