...
";
    ...
    private static string EmitCuUI(string ns, List<UIControlEntry> controls)
    {
        ...
        sb.AppendLine("        private static GUIStyle _style;");
        sb.AppendLine("        private static Dictionary<string, Texture2D> _texCache = new();");
        sb.AppendLine("        private static Dictionary<string, string> _texKey = new();");
        sb.AppendLine();
        ...
        sb.AppendLine("        {");
        sb.AppendLine("            InitData();");
        sb.AppendLine("            if (_style == null)");
        sb.AppendLine("            {");
        sb.AppendLine("                _style = new GUIStyle(GUI.skin.box);");
        sb.AppendLine("                _style.alignment = TextAnchor.MiddleCenter;");
        sb.AppendLine("            }");
        sb.AppendLine("            float fontScale = (float)Screen.height / 1080f;");
        ...
        sb.AppendLine("                int sw = Mathf.Max(1, (int)rect.width);");
        sb.AppendLine("                int sh = Mathf.Max(1, (int)rect.height);");
        sb.AppendLine("                var oldColor = GUI.color;");
        sb.AppendLine("                GUI.color = new Color(1f, 1f, 1f, c.Opacity);");
        sb.AppendLine("                var tex = GetCtrlTexture(c, sw, sh);");
        sb.AppendLine("                _style.normal.background = tex;");
        sb.AppendLine("                _style.hover.background = tex;");
        sb.AppendLine("                _style.active.background = tex;");
        sb.AppendLine("                _style.onNormal.background = tex;");
        sb.AppendLine("                _style.onHover.background = tex;");
        sb.AppendLine("                _style.onActive.background = tex;");
        sb.AppendLine("                _style.fontSize = Mathf.RoundToInt(c.FontSize * fontScale);");
        sb.AppendLine("                _style.normal.textColor = c.TextColor;");
        sb.AppendLine("                _style.hover.textColor = c.TextColor;");
        sb.AppendLine("                _style.active.textColor = c.TextColor;");
        sb.AppendLine("                if (c.Type == \"button\")");
        sb.AppendLine("                    _pressed[c.Id] = GUI.Button(rect, c.Text, _style);");
        sb.AppendLine("                else if (c.Type == \"textfield\")");
        sb.AppendLine("                    c.TextFieldText = GUI.TextField(rect, c.TextFieldText, _style);");
        sb.AppendLine("                else if (c.Type == \"toggle\")");
        sb.AppendLine("                    c.ToggleState = GUI.Toggle(rect, c.ToggleState, c.Text, _style);");
        sb.AppendLine("                GUI.color = oldColor;");
        sb.AppendLine("            }");
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        static Texture2D GetCtrlTexture(Ctrl c, int w, int h)");
        sb.AppendLine("        {");
        sb.AppendLine("            string key = c.Id + \"_\" + w + \"x\" + h + \"_\" + c.BgColor.ToString() + \"_\" + c.StrokeColor.ToString() + \"_\" + c.StrokeWidth + \"_\" + c.CornerRadius;");
        sb.AppendLine("            string oldKey;");
        sb.AppendLine("            if (_texKey.TryGetValue(c.Id, out oldKey) && oldKey == key && _texCache.ContainsKey(c.Id))");
        sb.AppendLine("                return _texCache[c.Id];");
        sb.AppendLine("            var tex = MakeRoundedTex(w, h, c.BgColor, c.StrokeColor, (int)c.StrokeWidth, (int)c.CornerRadius);");
        sb.AppendLine("            _texCache[c.Id] = tex;");
        sb.AppendLine("            _texKey[c.Id] = key;");
        sb.AppendLine("            return tex;");
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        static Texture2D MakeRoundedTex(int w, int h, Color fill, Color stroke, int strokeW, int radius)");
        sb.AppendLine("        {");
        sb.AppendLine("            w = Mathf.Max(1, w);");
        sb.AppendLine("            h = Mathf.Max(1, h);");
        sb.AppendLine("            var tex = new Texture2D(w, h, TextureFormat.RGBA32, false);");
        sb.AppendLine("            var px = new Color[w * h];");
        sb.AppendLine("            for (int y = 0; y < h; y++)");
        sb.AppendLine("            {");
        sb.AppendLine("                for (int x = 0; x < w; x++)");
        sb.AppendLine("                {");
        sb.AppendLine("                    if (!InsideRounded(x, y, w, h, radius))");
        sb.AppendLine("                    { px[y * w + x] = new Color(0,0,0,0); continue; }");
        sb.AppendLine("                    if (strokeW > 0 && !InsideRounded(x - strokeW, y - strokeW, w - 2 * strokeW, h - 2 * strokeW, Mathf.Max(0, radius - strokeW)))");
        sb.AppendLine("                    { px[y * w + x] = stroke; continue; }");
        sb.AppendLine("                    px[y * w + x] = fill;");
        sb.AppendLine("                }");
        sb.AppendLine("            }");
        sb.AppendLine("            tex.SetPixels(px);");
        sb.AppendLine("            tex.Apply();");
        sb.AppendLine("            return tex;");
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        static bool InsideRounded(int x, int y, int w, int h, float r)");
        sb.AppendLine("        {");
        sb.AppendLine("            if (r <= 0) return true;");
        sb.AppendLine("            if (x < 0 || y < 0 || x >= w || y >= h) return false;");
        sb.AppendLine("            if (x < r && y < r) { float dx = r - 1 - x, dy = r - 1 - y; return dx*dx + dy*dy <= r*r; }");
        sb.AppendLine("            if (x >= w - r && y < r) { float dx = x - (w - r), dy = r - 1 - y; return dx*dx + dy*dy <= r*r; }");
        sb.AppendLine("            if (x < r && y >= h - r) { float dx = r - 1 - x, dy = y - (h - r); return dx*dx + dy*dy <= r*r; }");
        sb.AppendLine("            if (x >= w - r && y >= h - r) { float dx = x - (w - r), dy = y - (h - r); return dx*dx + dy*dy <= r*r; }");
